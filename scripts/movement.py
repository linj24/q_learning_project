...
class MovementController(object):
    ...
    def calculate_velocity_scan(self, scan_data: LaserScan) -> Twist:
        bot_vel = Twist()
        scan_ranges = np.array(scan_data.ranges)
        if not np.isinf(scan_ranges[0]):
            distance_to_object = scan_ranges[0]
        else:
            distance_to_object = min(
                scan_ranges[0:C.FRONT_ANGLE_RANGE//2], scan_ranges[-C.FRONT_ANGLE_RANGE//2:])
    ...
    def calculate_velocity_img_cen(self, img_cen: ImgCen) -> Twist:
        bot_vel = Twist()
